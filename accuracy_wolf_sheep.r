...
acc_wolf_sheep = data.frame(prolific_id = character(), acc_wolf_sheep = double(), stringsAsFactors = FALSE)
...
for (i in 1:length(prolific_id)) {
  ...
  acc_wolf_sheep[i, 1] = prolific_id[i]
  acc_wolf_sheep[i, 2] = scales::percent(h / 48)
}

write.table(acc_wolf_sheep,
            file = paste0('acc_wolf_sheep', '.csv'), sep = ",", col.names = TRUE,
            row.names = FALSE, qmethod = "double")
